"""Typer entrypoint for the mcp-config-audit CLI."""
...
from mcp_config_audit import __version__, report
from mcp_config_audit.discovery import HOST_UNKNOWN, ConfigLocation, find_all_configs
from mcp_config_audit.parsers import MCPServer, parse_config_file
from mcp_config_audit.report import Report, SEVERITY_STYLES
from mcp_config_audit.rules import Finding, Severity, load_rules, run_rules
...
@app.callback()
def main() -> None:
    """Keep mcp-config-audit a command group even while only one command exists."""


@app.command()
def version() -> None:
    """Print the mcp-config-audit version."""
    console.print(f"mcp-config-audit {__version__}")
